// Portfolio Lightbox Gallery
  ...
  function applyProjectVisibility() {
    ...
    const cards = grid.querySelectorAll('.project-card:not(.project-card-behance)');
    ...
    if (moreWrap) moreWrap.classList.toggle('hidden', projects.length <= visibleCount);
  }
  ...
  function buildDots() {
    ...
    const total = Math.ceil(REVIEW_COUNT / getVisible());
  ...
  function goToReview(index) {
    ...
    const total = Math.ceil(REVIEW_COUNT / visible);
    currentReview = ((index % total) + total) % total;
    cardOffset = REVIEW_COUNT + currentReview * visible;
    applyTransform(true);
    updateDots();
  }

  // Arrow / autoplay step — scrolls into the clone, then snaps back
  // once the transition ends.
  function moveReviews(dir) {
    const visible = getVisible();
    const total = Math.ceil(REVIEW_COUNT / visible);

    cardOffset += dir * visible;
  ...
  window.addEventListener('load', () => { buildDots(); applyTransform(false); startReviewsAutoplay(); });
